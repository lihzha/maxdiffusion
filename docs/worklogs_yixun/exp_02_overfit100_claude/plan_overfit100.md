...
- **V2 — stats envelope**, every built window: finite; **std ∈ [0.25, 1.25]; |mean| ≤ 0.30** *(recalibrated 2026-07-30 after full-build job 060037 aborted on ep3905 — a legitimate high-contrast scene at std 0.951–0.958 with V1/V3/V4 all passing; original bounds [0.35, 0.95]/0.15 came from a thin sample: cache 0.60–0.70, probe2 0.737–0.803. The failure modes V2 exists to catch — missed normalization ≈16× or ≈0.06× std, NaN, dead encodes — remain orders of magnitude outside the new bounds; reviewed in the v2-envelope mini-round)*.
