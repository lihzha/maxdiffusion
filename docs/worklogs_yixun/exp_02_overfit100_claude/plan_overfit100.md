...
- **Coverage:** intermediate checkpoints → canonical windows (median window per episode: `start = 4·⌊(n_w−1)/2⌋`), 1 seed, correct text. **Three-seed checkpoints** (S2: its own {250,500,1000,2500} over the 10-episode cohort — feeding the D10 gate only; S3: segment finals) → 3 fixed seeds + the 3 text modes. Final candidate checkpoint → **all** built windows, 1 seed, correct text.
