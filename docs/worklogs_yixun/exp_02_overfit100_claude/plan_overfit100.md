# plan_overfit100 — exp_02: text-conditioned 100-trajectory memorization test (v3)

Planner: Claude Fable 5 (xhigh; v1 under Opus 5 max — deviation logged). Status: **v3 after second Codex review** (v1 → F1–F7, v2 → G1–G5; both reviews + resolutions in `overfit100_codex_plan_review.md` / `overfit100_codex_plan2_review.md`). Data path **A′** (Query 3). Base: `claude-exp_02_overfit100-20260728` (exp_01 code merged in; `yixun-dev` untouched). v1 @ `cb5d73f`, v2 @ `092eb91`.
...
- **V1 — round-trip vs cache** on the three cached windows already held locally (`ep0_v0_s00000/s00004/s00008`): decode cached `z_video` → re-encode via our path → require relative L2 `‖ẑ−z‖₂/‖z‖₂ ≤ 0.25` **and** Pearson r ≥ 0.97 per window. Catches normalization/layout errors (a missed mean/std shows as ~16× std blowup) while tolerating VAE round-trip loss.
...
- **Coverage:** intermediate checkpoints → 100 canonical windows (median window per episode: `start = 4·⌊(n_w−1)/2⌋`), 1 seed. **Three-seed checkpoints C₃ = every segment-final checkpoint** (S2: 2500; S3: 2500, and 5000/7500/10000 if extended) → 3 fixed seeds + the 3 text modes. Final candidate checkpoint → **all** built windows, 1 seed, correct text.
...
- **Success rule (exact):** for `c ∈ C₃` and canonical window `w`: `m(w,c) = median_{seed∈{0,1,2}} SSIM(w,c,seed)` (median over seeds first, threshold second). **Memorization established iff `max_{c∈C₃} fraction{w : m(w,c) ≥ 0.95} ≥ 0.90`**; partial at 0.90 threshold. Denominator = all 100 canonical windows, fixed at build (collisions flagged, never dropped). **Scope guard (G4):** the all-window single-seed eval at the best `c` is a robustness check — if `fraction{SSIM ≥ 0.90}` over all windows < **0.75**, the claim is explicitly narrowed to "canonical-window memorization." **The evaluator writes the aggregation artifact** (per-window/per-seed SSIM, `m`, fractions, verdict inputs) as JSON — no hand computation.
...
- **Tests:** selection determinism + empty-filter + draw-log; manifest fingerprint-drift fail; window math (ep0→14, ep1→24, edges); schema round-trip; **train10 filter + count assert (G3)**; text-table parity/order/no-negatives/bytes; `_data_shardings` tree-match; **row-distinct gather with index≠id fixture (G1)**; objective parity vs full-FT on null-equal table row; context modes (null ≡ exp_01 path, shuffled = seeded derangement); gate V1–V4 threshold logic on fixtures; **success-statistic function (m(w,c), fractions, guard) as a pure tested unit (G4)**.
