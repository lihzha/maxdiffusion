...
- Hardware: v6e-64, us-east1-d; pure FSDP (`ici_fsdp_parallelism=-1`); per-device batch 8 ⇒ **GBS 512**
- Steps: **10000** (≈3.55 passes over 1,440,554 windows); LR **1e-5** AdamW (b1 .9, b2 .999, eps 1e-8, wd 1e-2, global-norm clip 1.0, warmup fraction 0.05)
...
- **Evaluation points:** step-0 pretrained baseline (`checkpoint_step=0` bypass) + checkpoints 2500/5000/7500/10000, all via `bash_scripts/validate_wan_full_ft.sh` on v6e-8, metrics latent MSE / pixel MSE / SSIM + comparison videos; success = within-cohort delta vs step-0 baseline.
