...
## 2026-07-27T02:59:56Z — PRE-LAUNCH PACKAGE (Part II) + smoke launch under the Query-9 grant

- **Goal** — Record the full package; launch T1 smoke (dual sign-off complete: Codex = cycle-C review verdict; Fable = 213+2 suite + statics + rung-3 PASS + pushed `6b6b80f`).
- **Jobs & topology:** 3 × v6e-8 (us-east1-d queue). (1) T1-smoke: 1 checkpoint (2500), SMOKE_LIMIT=4 batches, output `validation_loss_smoke/` — exercises the two-host process_allgather path; (2) T1-full: 8 checkpoints SEQUENTIAL in one job (plan D4: parallel = 8× setup/model-load for ~40 min saved; sequential also trivially guarantees identical data/RNG); (3) T2-viz: step-20000 rollout on 6 val positions (reuses the Part-I-proven generate path; gated on smoke only for queue discipline, not code risk).
- **Runtime/cost estimates:** smoke ~35–50 min wall; T1-full ~1.2–1.7 h (8 × [restore ~2-3 min + eval ~2-4 min] + setup/load ~25 min); T2 ~35–50 min. Total ≈ 20–25 chip-hours (vs ~300 for the training run).
- **Commits:** eval code `6b6b80fee0deca9a460ee4d1957749d6add93b86` (pushed); TRAIN_COMMIT stamped into every row: `031228ee926d189a07396336e68c27b7a278f96c` (the training run's SHA).
- **Output paths:** T1 `gs://v6_east1d/checkpoints/maxdiffusion/wan-ti2v-full-ft/wan-full-ft-v6e64-full-gbs256-fresh-20260719-165222/validation_loss/{val_loss.json,val_loss.csv,val_loss_plot.png}` (smoke: `validation_loss_smoke/`); T2 `…/wan-full-ft-v6e64-full-gbs256-fresh-20260719-165222/validation_valset/step_020000/` + local `gallery.html` via `make_wan_val_gallery`.
- **Deterministic RNG method:** per-example key = fold_in(key(validation_seed=0), dataset_position); split → t (randint int32 uniform over the 25-point training sigma grid: build_rollout_sigmas(25, flow_shift 5.0, scheduler σmin/σmax)) and ε (normal, unbatched example shape, f32) — checkpoint/batch/order/host-invariant by construction; distribution parity with training, not draw replay. Position ≡ stored ordinal on this split (rung-3), but the code indexes position by contract.
- **Padding exclusion:** batches of 32 (per-device 4 × 8); 14,636 = 457×32 + 12 → last batch pads by repeating the final record with validity=False; per-example losses gathered per batch (multihost allgather) and aggregated over VALID entries only (golden-tested exclusion from mean AND stderr); reader drained to EOF with hard n==14,636 assert BEFORE any restore.
- **Acceptance criteria — T1 smoke:** worker COMMIT=6b6b80f…; 8 devices/2 hosts; TRAIN_COMMIT guard passes; drain asserts 14,636; checkpoint 2500 restore (returned step == state.step == 2500); 4 batches × allgather; smoke JSON/CSV rows in `validation_loss_smoke/`; JOB_EXIT=0; NO writes to `validation_loss/`.
- **Acceptance criteria — T1 full:** all 8 restores at exact steps; n==14,636 × 8; 9-column JSON+CSV at the declared path; plot PNG present (worker or recorded local regeneration) BEFORE acceptance; rows carry both SHAs.
