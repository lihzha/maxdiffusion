...
Reviewer: OpenAI Codex `gpt-5.6-sol` xhigh. Verdict: REQUEST-REVISION — **1 BLOCKER + 2 MAJOR, all accepted.** (a) additive purpose PASS (old ids unchanged, oracle genuinely independent, T1's 40 tests untouched); (e) U08's message-matching ACCEPTED as not over-fitting. **BLOCKER: the stream omits ** — the settled C0 objective samples per-example timesteps from  (), so leaving it outside `StepDraws` makes it accumulation- and resume-dependent and would silently confound the R-B-vs-C0 comparison. **MAJOR: the accumulation-invariance test is vacuous** (deletes the parameter and repeats an identical call) and the arbitrary-shape API lets a per-microbatch draw give epsilon EXACTLY unequal to the factor-1 draw at factors 2/4/8 — the reviewer demonstrated it; a draw-once-at-logical-shape-then-split seam is required. **MAJOR: the restored-`state.step` obligation is NOT discharged** at caller level (any caller may still pass `state.step`; the demonstration's opening-stream equality is tautological and exercises neither a state nor the loop) — it stays OPEN for T3b-4's integrated oracle with a production-callsite AST pin.

**Planner note:** the BLOCKER was relayed to the Coder MID-FLIGHT because it changes what T3b-2's C0 arm consumes (`t_idx` must come from the stream, not be drawn in the arm) — folding it in beats reworking it. Verdict (d) is downgraded to "primitive statelessness only"; S7's loop-level resume failure class remains live until T3b-4.
