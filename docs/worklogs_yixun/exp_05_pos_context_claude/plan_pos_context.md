...
Chosen arm over TRAIN-2000 + DEV-64 + TEST-64. Schema = exp_04's with `pos_embeds [25,8,4096]` and **`z_bar_states [25,48,9,12,20]`** (both `latent_dtype`-governed). **Storage (F7, corrected):** states ≈ 5.93 MiB/record fp16; full record ≈ 8.2 MiB; 2,128 records ≈ **17.1 GiB** (fp32 fallback ≈ 34 GiB) — free-space check against the bucket before the build. **Integrity (F7):** byte-length validation and completion-marker fingerprints explicitly cover `z_bar_states`. **Fidelity gate extension (F7; expectation corrected by the S5 measurement — scope-neutral amendment):** on the predeclared 8-DEV subset, serialized-fp16 states are compared as bf16 model inputs against in-memory fp32 states. ~~Expected to pass trivially~~ — **S5 proved the value-preservation premise FALSE**: fp32→fp16→bf16 double-rounding diverges from the direct bf16 cast on ≈6.2% of latent-like elements (always exactly 1 bf16 ulp; constructed witness x=1+2⁻⁸+2⁻¹¹; plus fp16 range destruction at |x|>65504 and |x|<~6e-8). The bit-identity path is therefore expected to FAIL on real data; **the pinned block-0 feature tolerance (max|Δ| ≤ 1e-2) is the load-bearing path**, and when feature deltas are unavailable the gate conservatively selects fp32. Decision structure unchanged. Storage figures superseded by exact arithmetic: fp16 ≈ 7.12 MiB/record, cohort ≈ **14.80 GiB** (fp32 fallback ≈ 29.1 GiB) — consistent with the plan reviewer's cross-check. Cross-example structure diagnostic (cosine/PCA of C*) computed on **TRAIN + DEV only; TEST targets are written but uninspected until K4** (F7).
