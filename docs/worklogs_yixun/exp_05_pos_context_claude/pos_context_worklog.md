...
## 2026-08-08T07:35:00Z — P4' audit corrections (append-only; report Coder vs primary artifacts)
...
## 2026-08-09 — Post-STOP capacity-videos runbook + job (P4/P4′ report support, both experiments)

- **What** — Yixun asked to *see* what both methods produce; approved a ~1 h v6e-8 replay+decode job. New runbook renders the never-decoded capacity arms as GT-top/pred-bottom mp4s: exp_04 **A2** + **A1-probe k=0**, exp_05 **B1** + **B2** + **B1-probe k=0**, 8 clips each. Decode-only arms come straight from each record's `expected_final_latent` (the cast→replay→record discipline makes that the deployed latent); probe arms are true 25-step replays of the stored conditioning from `keyed(name, 0)` starts, each through its own slot's operators (`replay_with_nulls` uncast-backend / `replay_with_positive` via `casting_velocity_fn`, both idempotent over `_load_backend`'s cast).
- **Code** — commit `6f3146a` on this branch: `capacity_videos.py` (seam-injected orchestration), `run_wan_capacity_videos.py` (+ `pos_read_shard`), launcher `bash_scripts/run_wan_capacity_videos.sh`, six `videos_*` YAML keys, `test_capacity_videos.py` (10 CPU tests; a zero-velocity end-to-end pin of keyed-noise + first-frame-pin through the REAL replay operators); the YAML-pinning test extended for the `videos_*` block. All small-artifact writes are overwrite-idempotent to attempt-scoped roots pinned at submission — issue #13's shard-immutability trap structurally cannot fire.
- **Job** — `20260809-173808-13c3cadc-capvideos-yixun`, v6e-8 us-east1-d, **SUCCEEDED** at attempt 2 (one infra retry, absorbed by the idempotent design; ~7 min scheduler-ingest lag before the job appeared in `tpu status` — submission itself was fine). Outputs: `gs://v6_east1d/datasets/droid_wan_null_adapter/j1r2/videos_att-0809-173808/` (16 mp4s) and `gs://v6_east1d/datasets/droid_wan_pos_context/k1/videos_att-0809-173808/` (24 mp4s), each with a `videos_report.json` carrying provenance + per-clip recomputed-vs-published future-SSIM.
- **Cross-check** — CLEAN: every arm's recomputed per-clip future-SSIM matches the published `gate_tables.json` within fp16-storage tolerance (max |Δ| 0.052 on b2; means +0.002–0.006); provenance binds `3bdbd2a` (J1-4) / `bb845ea` (K1-2), same model revision. 8-clip means: a2 0.469, a1_probe_k0 0.167, b1 0.923, b2 0.178, b1_probe_k0 0.439 — the published story, now visible.
- **[DEVIATION — RECORD]** The rendered subset is the **8 lowest-ordinal DEV clips**, not the first-8-by-manifest-row behind J1-4's published A1 videos: `subset_records` sorts by the record's `ordinal`, which is a global dataset ordinal, not row position (my docstring's "reproduces decode_subset" claim was wrong). All 8 are valid DEV-64 clips; the five new arms are same-clip comparable with each other; overlap with the A1 videos = {ep12399, ep19599, ep21099}. A row-order re-render is a cheap optional follow-up, Yixun's call.
- **Local viewing** — `artifacts_local/capacity_videos_report.html` (combined, both experiments + the J1-4 A1 videos with per-clip tables; `artifacts_local/` now gitignored). Feeds directly into the outstanding P4/P4′ HTML reports.

## 2026-08-10T (local) — P4′ reports revision 3: closing review applied; reports FINAL, submitted for closure adjudication

- **Goal** — Apply `pos_context_codex_closing_review.md` (REQUEST-REVISION; **"the exp_05 STOP and gate record remain valid"**; two scoping leaks + the exp_04-J=50 fold-in + artifact-index staleness; no new compute). Docs-only.
- **Result** — `passed`. All four applied; both reports flipped **DRAFT → FINAL (revision 3)**.
- **Item disposition:**
  1. **Two scoping leaks closed.** Analysis §1's "Capacity is not the limit" → **"output-channel expressivity is not limiting on these clips; head function capacity remains untested"**; the summary's "a per-clip conditioning tensor is the wrong object to regress onto" → **"the tested single-basin greedy-pivot cached-target family was rejected for K2/K3"**, with robust/multi-noise static targets and state-conditioned emitters named as untested; and the unsupported extrapolation *"on the full cohort the positive probe would likely lead"* **deleted** (no 64-clip joint-null arm exists).
  2. **exp_04's terminal J=50 result folded in everywhere it had been treated as J=10-only.** results §1's "Unlike exp_04" removed; results §4.3 and analysis §5.2 rewritten — the comparison is **recipe-matched at J=50**, the budget mismatch and all INDETERMINATE/pending language are gone, and **three mismatches remain (control, representation, pivots) and are sufficient**; the fresh-noise table now reads exp_04 **4.743 → 0.303 = 15.7× better** vs exp_05 **4.0× worse**; in-basin **0.9227 vs 0.8868, gap 0.036**; exp_04's G1 ratio **4.681×** (3.605× retained as explicitly historical) in analysis §5.2 and results caveat 4; §5.3's null figures → **A2 0.6638 / A1-probe 0.1666**; the launcher wording historicalised (J1-4 failed adoption, the `adequacy-wiring` fix `a520e9d` landed, J1-5 ran clean).
  3. **J=10-era evidence preserved and labelled.** The matched-eight joint-null result (**0.730 retention / 0.4753 absolute**) was **not** overwritten — the exp_04 rows in analysis §4.1 and the `~19×` statement in §7.2 are now explicitly marked **J=10-era J1b/J1c evidence**, with a note that the J=10-vs-J=50 caveat still applies to *them* even though §5.1's clean-gate comparison is now J=50-matched.
  4. **Artifact index brought up to date.** The capacity-videos job `20260809-173808-13c3cadc-capvideos-yixun` (**24 mp4s** — B1, B2, B1-probe k=0 — at `…/k1/videos_att-0809-173808/`, cross-check **CLEAN**, 8-clip means b1 0.923 / b2 0.178 / b1_probe_k0 0.439) and both committed HTML pages are now recorded, together with the **lowest-ordinal-subset deviation**; tip row updated (`0f505d3` / `2e4bc12` / `d9a306e` / `6f3146a`, K1-2 evidence tip `bb845ea`); analysis threat 5 and next-step 3 rewritten from "nothing exists" to "exists, cross-checked, with two residual caveats".
- **Verification** — Every folded exp_04 figure re-derived from the cached J1-5 artifacts before writing: A1 0.88678, A2 0.66382, A1-probe 0.16659, A2 MSE 0.30255, A2-0 MSE 4.74311 ⇒ ratio **15.68×**; exp_05 B1 0.92269, B2 MSE 5.62498, B2-0 MSE 1.41775 ⇒ **3.97×**; in-basin gap **0.0359**. All match the rounded figures written.
- **Analysis** — The residual honest gaps, both recorded rather than smoothed: the rendered video subset is the 8 **lowest-ordinal** clips (not a row-order subset), and **no systematic human qualitative review is on record** — so "a systematic visual artifact could hide behind these means" is *reduced*, not eliminated.
- **Next** — **Yixun's closure adjudication** (announcement 03). Tracker + CLAUDE.md still to carry: *"the planned single-basin greedy cache target was rejected; state-conditioned live rollout training remains untested and risky, but was not refuted."*
