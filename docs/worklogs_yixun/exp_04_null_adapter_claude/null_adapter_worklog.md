...
## 2026-08-10T (local) — P4 reports revision 3: J1-5 folded in; the formal verdict is MEASURED

- **Goal** — Fold the completed clean-gate rerun into `null_adapter_results.md` + `null_adapter_analysis.md`, replacing revision 2's "supersede-by-the-rerun" scaffolding with the measured outcome. Docs-only.
- **Verification** — Every J1-5 number re-derived from the published artifacts, not from the ledger: `cleangate_att-0809-211639/{selection,gate_tables,run_report}.json`, `cleangate_trainfit_att-0809-211639/`, `cleangate_att-0809-180640/` (attempt 1), and `a1/shard_00000/header.json`. All 4 ledger figures matched exactly. **Two claims verified as STRONGER than the ledger stated:** (1) attempt 1 vs attempt 2 are not merely equal "to 4+ decimals" — the `g1`, `g2` and `selection` number blocks are **byte-for-byte equal objects**; (2) A0 and A2-0 are not merely "identical" to J1-4 at the mean — they are **per-example bitwise identical**, three days and two code tips apart.
- **Result** — `passed`. **Formal verdict: `target = STOP`, measured at the adopted J=50/lr=0.01 on BOTH cohorts.** Revision 2's INDETERMINATE ruling is discharged by measurement.
- **Changes** — results: §1 status table rebuilt around FORMAL VERDICT / prior-observation / closure-adjudication rows; **new §4.5** (the full J1-5 record: provenance, job trail, reproduction, per-arm table, gate tables, robustness, cohort consistency, and §4.5.3 on the A2-vs-A0 comparison being cross-arm); §4.1 marked historical; §4.2/§4.4 indeterminacy resolved with the withdrawn reasoning preserved; J1-5 row added to the job trail; artifact index and caveat 4 updated; L_null/A3 subsections renumbered §4.6/§4.7. analysis: §1 rewritten to the measured verdict; §2.1/§2.2 restated at J=50 with J=10 parenthetical; §2.3 updated (the probe is now *measured* budget-flat); **new §2.4** (A2 converged to A0, not past it); §4.1 rebuilt as a point-by-point resolution table; §5.1/§5.2 recipe-matched; §6.1/§6.2, §7 priorities, §8 open questions.
- **Analysis — three readings worth carrying:**
  1. **The probe is budget-flat (0.1729 → 0.1666 over 5×).** The reviewer was right that the J=10 artifacts could not *entitle* anyone to assume this; the measurement now shows it is *true*. Revision 1 was **right for unavailable reasons** — the record keeps both halves, because being right without the evidence is still an evidence error.
  2. **A2 converged TO doing-nothing, not past it** (0.6638 vs A0's 0.6665) — a large 5×-budget gain that terminated at the quality of not optimizing in the right basin, still 0.086 short of the G2 bar. Flagged explicitly as a **cross-arm, unmatched** comparison (A2's matched control is A2-0, which it beats 15.7×); new hypothesis + cheap budget sweep recorded.
  3. **The greedy probes moved DOWN at 5× budget** (A2-probe 0.2958 → 0.2510). This runs *against* the analysis review's counter-mechanism for the §3.2 compute confound. **I did not touch §3.2's hedged conclusion** — A3 retains ~10× more compute than J=50 greedy and two points cannot establish monotonicity — but recorded it as an explicit, clearly-bounded pointer and made it open question #1 for the next review.
- **Next** — Yixun adjudicates closure (announcement 03). Then the P4 HTML report from the published `videos/`; optional J≈200 greedy probe (§3.2) and A2 budget sweep (§2.4).

## 2026-08-10T (local) — P4 reports revision 4: closing review applied; reports FINAL, submitted for closure adjudication

- **Goal** — Apply `null_adapter_codex_closing_review.md` (REQUEST-REVISION; **"the formal STOP at J=50 is valid and closure-grade"**; all six items wording/scoping/staleness; no new compute). Docs-only.
- **Result** — `passed`. All six applied; both markdown reports flipped **DRAFT → FINAL (revision 4)**.
- **Item disposition:**
  1. **Reproduction claim narrowed to DEV-64.** Attempt 1 published DEV-64 only; TRAINFIT-16 completed only in attempt 2. Standing wording: *"STOP measured on both cohorts; DEV-64's gate/selection blocks reproduced byte-for-byte across two attempts."* Code-state rows corrected to the `adequacy-wiring` tip and **991 tests** (was the pre-fix 989).
  2. **§3.2 J=50 integration, hedge preserved.** Removed the now-false "only ever measured at one budget" / "was never run"; replaced "budget-independent over this range" with *"neither cohort-wide probe improved between the measured J=10 and J=50 endpoints"*; outstanding experiment redefined as a **compute-matched J≈200+** probe. **Derived the cohort-matched figure the reviewer asked for:** the J=50 A2-probe restricted to J1c's first eight clips is **0.2220** (from the published `gate_tables.json`), vs joint's 0.4753 on the same clips — replacing the DEV-64-vs-n=8 comparison revision 3 made.
  3. **Convergence wording** → "landed near, at J=50"; the matching ledger correction already exists in `null_adapter_command.md` (which also withdraws "statistically indistinguishable") and is **referenced, not duplicated**.
  4. **Memorization overclaim removed.** "No memorization gap" / "not a dev-set artifact" → *"the same gate-failure signature occurred in both sampled cohorts; the observed A1 means were nearly equal"* / *"not unique to DEV-64"*.
  5. **Stale contradictions fixed:** results §4.4's budget-differ claim; analysis §5.1's "also differ in recipe"; §5.2's 9.7× → authoritative **15.7×** with "would need the recipe held constant" removed (the condition is now met); §5.3's 3.6× → **4.681×**, 3.6 retained as explicitly historical.
  6. **Closure/publication state:** §4 retitled (J1-4 is no longer "the primary result"); correction table "J=50 re-run pending" → **DISCHARGED**; artifact index now records the **capacity-videos job** (`20260809-173808-13c3cadc`, 16 mp4s, cross-check CLEAN) with its **lowest-ordinal-subset deviation** and the three committed HTML reports; the two HTML pages advertising "INDETERMINATE pending J1-5" updated in place (`01-capacity-gates` now carries the J=50 gate summary and a RESOLVED banner; `03-video-gallery` relabelled "videos render the J=10-era arms · formal verdict STOP @ J=50"); the review's adjudications recorded in analysis §8.
- **Analysis** — Two things worth carrying. (i) The videos and HTML render **J1-4 (`3bdbd2a`) provenance**, i.e. the **J=10-era** arms; the J=50 verdict has no video. Recorded in the artifact index and raised as a closure question rather than silently left. (ii) J1b/J1c were never re-run at J=50, so the mechanism sections still rest on J=10-era greedy baselines even though the verdict is J=50 — stated in results §9 caveat 4 and analysis §8.
- **Next** — **Yixun's closure adjudication** (announcement 03). Three questions are queued for him in analysis §8: whether to fund the J≈200+ compute-matched probe before closing, whether a J=50 video re-render is wanted, and whether the J=10-era mechanism sections are acceptable in a closed report.
