...
### 16. `us-east1-d` v6e-8 `TPU_VM_HEALTH_TIMEOUT` cluster, 2026-08-09 evening (infra, ALARM-adjacent)
- **Symptom:** two health-timeout kills across two unrelated jobs within an hour — exp_06 M1 attempt 2 at 1h56m (mid-XLA-compile, silent) and exp_04 J1-5 attempt 1 at 2h57m (mid-trainfit, after publishing capacity). Different workloads, different phases ⇒ zone flakiness, not our programs. M1 attempt 1 was additionally a plain `TPU_VM_PREEMPTED` pre-start.
- **Handling:** queue classifies both `INFRASTRUCTURE_PREEMPTION`/retryable and auto-retries (allowed — no code change). Attempt-scoped roots (issue #13) prevented all artifact collisions.
- **Cost lesson:** J1-5's retry re-runs the WHOLE runbook — attempt-scoped roots deliver correctness but the submit script implements no phase-level adoption of already-published artifacts (issue #13's second clause), so an already-published ~3 h capacity phase is being recomputed before trainfit. For multi-phase runbooks expected to run >2 h in this zone, either implement adopt-if-published preflights per phase or split into one queue job per phase.
- **Status:** OPEN (watch); if health timeouts recur on healthy workloads across days, raise with IROM-lab (queue-side knob, not ours).
