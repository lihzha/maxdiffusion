...
def _rows(names=None, *, num_steps=25, **fields):
    names = anchor.HISTORICAL_ANCHOR.sample_names if names is None else names
    row = {"latent_mse": 1.496, "pixel_mse": 0.0983, "ssim_avg": 0.2946, "num_steps": num_steps}
    row.update(fields)
...
def test_the_summary_is_the_deployed_aggregation():
    ...
    rows = [
        {"name": "a", "latent_mse": 1.0, "pixel_mse": 0.10, "ssim_avg": 0.20, "num_steps": 25},
        {"name": "b", "latent_mse": 2.0, "pixel_mse": 0.20, "ssim_avg": 0.40, "num_steps": 25},
    ]
...
def _execution(z_pred, *, num_steps=25):
    return anchor.RolloutExecution(
        z_pred=z_pred, num_steps=num_steps, grid_size=26, guide_scale=5.0, draw_key_sha256="0" * 64
    )
...
def _anchor_rows(names=None, num_steps=25):
    names = anchor.HISTORICAL_ANCHOR.sample_names if names is None else names
    return [
        {"name": n, "latent_mse": 1.496, "pixel_mse": 0.0983, "ssim_avg": 0.2946, "num_steps": num_steps}
        for n in names
    ]
