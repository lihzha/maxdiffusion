...
def main(
    ...
    *,
    ...
    # (review, finding 3). The null branch is exp_04's expression verbatim (merge-2: the fix round
    # made null_adequacy_uri a DIRECT read — YAML-declared, so a missing key fails loudly).
    adequacy_uri = (
        str(optional_config_value(config, "pos_adequacy_uri", ""))
        if positive is not None
        else str(config.null_adequacy_uri)
    )
    adoption = (
        pos_adoption(
            adequacy_uri,
