...
# The DEPLOYED sigma grid's own parameters (``base_wan_5b_side_adapter.yml`` + the Wan scheduler).
# A grid is not identified by its LENGTH: the reviewer passed an all-ones 26-sigma grid with terminal
# sigma 1.0 and 25 zero timesteps, and it was accepted and labelled a deployed 25-step execution.
DEPLOYED_FLOW_SHIFT = 5.0
DEPLOYED_SIGMA_MIN = 0.0
DEPLOYED_SIGMA_MAX = 1.0
DEPLOYED_NUM_TRAIN_TIMESTEPS = 1000
#: The dtype deployment casts every rollout input to before drawing noise. ``weights_dtype`` in the
#: pinned config is bfloat16, and the native-bf16 draw is a COMPLETELY different sequence from a
#: float32 draw (T5a's A13 finding, turned back on this module by the re-review).
DEPLOYED_EVAL_DTYPE = "bfloat16"
...
def reproduce_anchor(measurement: "Measurement") -> AnchorVerdict:
    ...
    if str(measurement.payload.get("grid_sha256", "")) != DEPLOYED_GRID_SHA256:
        raise ValueError(
            f"this measurement was taken on grid "
            f"{str(measurement.payload.get('grid_sha256', ''))[:16]!r}, not the deployed "
            f"{DEPLOYED_GRID_SHA256[:16]}: the published 0.2946 is a fact about THAT schedule"
        )
    deviations, failed = {}, []
    for metric in ANCHOR_METRICS:
        # `abs(NaN) > tolerance` is FALSE, so a non-finite metric does not fail the band -- it passes
        # it. Both the value and its deviation are required to be finite BEFORE they are compared.
        value = require_finite(measurement.means[metric], f"the measured {metric}")
        recorded = float(getattr(HISTORICAL_ANCHOR, metric))
        deviations[metric] = require_finite((value - recorded) / recorded, f"the {metric} deviation")
...
def anchor_certificate(verdict: AnchorVerdict) -> dict:
    ...
    return {
        ...
        "grid_sha256": measurement.grid_sha256,
...
def deployed_grid():
    """THE grid the deployed evaluator advances along — built from its own pinned parameters.

    ``build_rollout_sigmas``/``rollout_timesteps_from_sigmas`` are the deployed constructions (T1's
    pinned blob), so this is not a description of the grid, it IS the grid, recomputed.
    """
    return overfit100_sampler_grid(
        num_inference_steps=DEPLOYED_SAMPLING_STEPS,
        flow_shift=DEPLOYED_FLOW_SHIFT,
        sigma_min=DEPLOYED_SIGMA_MIN,
        sigma_max=DEPLOYED_SIGMA_MAX,
        num_train_timesteps=DEPLOYED_NUM_TRAIN_TIMESTEPS,
    )


def grid_digest(sigmas, timesteps) -> str:
    """The identity of a rollout's grid, over the VALUES in float64 — not over their shapes."""
    body = np.concatenate(
        [np.asarray(sigmas, dtype=np.float64).ravel(), np.asarray(timesteps, dtype=np.float64).ravel()]
    )
    return hashlib.sha256(body.tobytes()).hexdigest()


def _deployed_grid_sha256() -> str:
    sigmas, timesteps = deployed_grid()
    return grid_digest(sigmas, timesteps)


def assert_deployed_grid(sigmas, timesteps) -> str:
    """Bind an execution to the deployed grid's VALUES, and return that grid's identity.

    Length was the only thing checked before, and length is the one property a forged grid gets right
    for free. Each clause below is a way a wrong grid stays the right shape: a terminal sigma that is
    not 0 never lands the rollout on the data manifold; a non-descending schedule runs the sampler
    backwards; timesteps that do not correspond to their sigmas feed the transformer a conditioning
    signal from a different point on the path. The digest then catches everything at once — the three
    named clauses exist so the refusal says WHICH property failed.
    """
    ...
    if not math.isclose(float(values[-1]), 0.0, abs_tol=0.0):
        raise ValueError(
            f"the terminal sigma is {float(values[-1])!r}, not 0: the deployed grid ends AT the data, and "
            f"a grid that does not is not the schedule the anchor was measured on"
        )
    if not np.all(np.diff(values) < 0.0):
        raise ValueError(
            "the sigmas do not descend strictly: the sampler walks from noise to data, so a flat or "
            "ascending schedule is not a 25-step deployed rollout however many entries it has"
        )
    expected_timesteps = np.asarray(values[:-1] * DEPLOYED_NUM_TRAIN_TIMESTEPS, dtype=np.float64)
    found = np.asarray(timesteps, dtype=np.float64)[:DEPLOYED_SAMPLING_STEPS]
    # float32 tolerance, deliberately: the deployed grid is built in float32, so `sigma * 1000`
    # recomputed in float64 differs in the last bits. The exact VALUES are pinned by the digest
    # below; this clause exists to name WHICH property failed, not to be the binding.
    if not np.allclose(found, expected_timesteps, rtol=1e-6, atol=1e-3):
        raise ValueError(
            "the timesteps do not correspond to their sigmas (deployment's "
            "`timesteps[i] = sigmas[i] * num_train_timesteps`): the transformer would be conditioned on "
            "a different point of the path than the state it is advancing"
        )
    digest = grid_digest(sigmas, timesteps)
    if digest != DEPLOYED_GRID_SHA256:
        raise ValueError(
            f"this grid is {digest[:16]}, and the deployed grid is {DEPLOYED_GRID_SHA256[:16]}: it has the "
            f"right shape and the right structure but different VALUES, so the rollout it drives is not "
            f"the measurement the anchor and the benchmark row were taken with"
        )
    return digest
...
class RolloutExecution:
    ...
    guide_scale: float
    draw_key_sha256: str
    #: The identity of the sigma/timestep VALUES this rollout advanced along (re-review EV-6). The
    #: grid was previously checked only for its LENGTH, which a forged grid gets right for free.
    grid_sha256: str = ""
...
#: Computed once from the deployed construction itself, so the pin cannot drift from the builder.
DEPLOYED_GRID_SHA256 = _deployed_grid_sha256()
...
def sample_metrics(execution: RolloutExecution, z_video, *, decode_fn, ground_truth=None) -> dict:
    ...
    **``ground_truth`` exists because deployment is ASYMMETRIC, and reproducing 0.2946 means
    reproducing that.** ``_rollout_sample`` computes the latent MSE against the ``weights_dtype``-CAST
    ``z_video`` (it is the cast array in that scope), while ``run()`` decodes
    ``batch["z_video"].astype(jnp.float32)`` — the ORIGINAL, uncast latents — for the pixel metrics
    and the SSIM. Passing one array for both would move the latent MSE by the bf16 rounding of the
    ground truth, which is exactly the kind of wiring difference the anchor exists to detect.

    **Every metric is checked for finiteness, not just the SSIM** (re-review EV-2): ``abs(NaN) >
    tolerance`` is false, so a NaN latent MSE certified as *reproduced*.
    """
    ...
    pixel_reference = z_video if ground_truth is None else jnp.asarray(ground_truth)
    ...
    gt0 = np.asarray(decode_fn(pixel_reference.astype(jnp.float32))[0], dtype=np.float32)
    ...
    return {
        # Each one goes through `require_finite`, at the point it is produced. A non-finite metric
        # cannot be compared, so downstream it does not fail the anchor -- it PASSES it silently.
        "latent_mse": require_finite(jnp.mean(diff**2), "the latent MSE for this sample"),
        "pixel_mse": require_finite(np.mean((pred0 - gt0) ** 2), "the pixel MSE for this sample"),
        "pixel_mae": require_finite(np.mean(np.abs(pred0 - gt0)), "the pixel MAE for this sample"),
        "ssim_avg": require_finite(ssim, "the SSIM for this sample"),
        "num_steps": int(execution.num_steps),
        "draw_key_sha256": execution.draw_key_sha256,
        "grid_sha256": execution.grid_sha256,
    }
...
class Measurement:
    ...
    def grid_sha256(self) -> str:
        return str(self.payload["grid_sha256"])
...
def summarize_samples(
    ...
    *,
    ...
    # EVERY metric, at EVERY level (re-review EV-2). SSIM was the only one checked, and a NaN latent
    # MSE therefore certified as reproduced: `abs(NaN) > tolerance` is false, so the band waves it
    # through instead of failing it. Checked per sample here and again on the aggregates below.
    for index, row in enumerate(rows):
        for field in ("latent_mse", "pixel_mse", "ssim_avg"):
            require_finite(row.get(field, float("nan")), f"sample {index} ({names[index]!r})'s {field}")
    grids = {str(row.get("grid_sha256", "")) for row in rows}
    if grids != {DEPLOYED_GRID_SHA256}:
        raise ValueError(
            f"these samples were rolled out on grids {sorted(g[:16] for g in grids)}; the deployed grid is "
            f"{DEPLOYED_GRID_SHA256[:16]}. A measurement is identified by the schedule it advanced along, "
            f"not only by how many steps it took."
        )
    ...
    payload = {
        ...
        "mean_latent_mse": require_finite(np.mean([float(row["latent_mse"]) for row in rows]), "mean_latent_mse"),
        "mean_pixel_mse": require_finite(np.mean([float(row["pixel_mse"]) for row in rows]), "mean_pixel_mse"),
        "mean_ssim": require_finite(np.mean([float(row["ssim_avg"]) for row in rows]), "mean_ssim"),
        "num_samples": len(rows),
        "num_steps": DEPLOYED_SAMPLING_STEPS,
        "grid_sha256": DEPLOYED_GRID_SHA256,
...
class DeviceBackend:
    ...
    def __init__(
        ...
        *,
        ...
        #: The cached reader hands this evaluator float32, so without the cast the anchor draws a
        #: float32 sequence where deployment drew a bf16 one. T5a's own A13 finding measured how far
        #: apart those are (`[0.387, 0.183, -1.0]` native vs `[1.625, 2.031, -0.434]` via fp32), so
        #: the anchor would then fail on WIRING and be read as model quality.
        self.eval_dtype = jnp.float32 if eval_dtype is None else jnp.dtype(eval_dtype)
    ...
    def score(self, *, z_i0, z_video, actions, key, adapter_enabled: bool = True):
        ...
        # Deployment's boundary, reproduced exactly: cast, THEN draw. The order is the whole point --
        # the noise is drawn in `z_video.dtype`, so casting after the draw is a different measurement.
        cast_i0 = jnp.asarray(z_i0).astype(self.eval_dtype)
        cast_video = jnp.asarray(z_video).astype(self.eval_dtype)
        cast_actions = jnp.asarray(actions).astype(self.eval_dtype)
        cast_context = jnp.asarray(self.context).astype(self.eval_dtype)
        ...
        # The latent MSE is measured against the CAST ground truth and the pixel metrics against the
        # ORIGINAL -- deployment's own asymmetry (`_rollout_sample` vs `run`), see `sample_metrics`.
        return execution, sample_metrics(
            execution, cast_video, decode_fn=self.decode_fn, ground_truth=jnp.asarray(z_video)
        )
...
def load_device_backend(config):  # pragma: no cover -- real Wan weights, a real VAE, a real device
    ...
    # Every config key read below is declared by `configs/base_wan_5b_pos_rollout.yml`, and a test
    # holds that contract over this function's ACTUAL reads rather than over a list someone kept.
    ...
    # `config.num_train_timesteps`, which no rollout YAML declares -- and on a real
    # `HyperParameters` an undeclared key RAISES (issue #11, `pyconfig.py:316`), so this loader died
    # before a single sample was scored and before the grid check below could ever run. It is the
    # same family as the getattr defect that killed exp_04's J1 smoke after the 5B was on device.
    ...
    sigmas, timesteps = overfit100_sampler_grid(
        ...
        sigma_min=float(scheduler.config.sigma_min),
        sigma_max=float(scheduler.config.sigma_max),
        num_train_timesteps=int(scheduler.config.num_train_timesteps),
    )
    # ...and the result is bound to the deployed grid HERE, at load, so a scheduler that disagrees
    # fails closed with a named difference instead of producing a plausible number per sample.
    assert_deployed_grid(sigmas, timesteps)
    ...
    return DeviceBackend(
        ...
        # `weights_dtype` is deployment's evaluation boundary, and it is bfloat16 in the pinned
        # config. Read from the config rather than pinned here so a run that changes it is measured
        # as it runs -- but a run that changes it is no longer comparable with the anchor, which the
        # published `eval_dtype` on every certificate makes visible.
        eval_dtype=_dtype(config.weights_dtype),
    )
