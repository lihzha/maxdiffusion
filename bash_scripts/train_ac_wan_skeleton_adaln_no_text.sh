...
# video is the ONLY conditioning signal and cross-attention gets all-zero tokens.
# The ablation that isolates how much of the action-following comes from the
# skeleton alone. See train_ac_wan_skeleton_adaln.sh for the text-ON arm, where
# the freed cross-attention carries the full 512-token T5 sequence.
#
...
python src/maxdiffusion/train_wan.py \
    ...
    run_name=ac_wan_droid_skeleton_adaln_no_text \
    ...
    wandb_project='wan-ac-skeleton-adaln-no-text' \
...
# tpu create v6 --name train_ac_wan_skeleton_adaln_no_text -n 32 --setup-cmd "" --priority 0 --max-attempts 40 -- bash bash_scripts/train_ac_wan_skeleton_adaln_no_text.sh
