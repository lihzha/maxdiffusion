...
# --- 5. Launch training ---
#
# Same as train_ac_wan_skeleton.sh but with use_task_instructions=False: the
# rendered skeleton video is the ONLY conditioning signal, cross-attention gets
# all-zero tokens. This is the OSCAR setup exactly, and the ablation that isolates
# how much of the action-following comes from the skeleton alone.
#
# action_cond_mode=skeleton: the 7-dim vector actions are NOT used at all.
# Conditioning is a rendered 2D kinematic skeleton video (robot proprioception ->
# URDF FK -> camera projection -> overlay), VAE-encoded with the SAME WAN VAE as
# the RGB video so its latents are token-for-token aligned. A separate zero-init
# patch-embedding conv projects them and the result is ADDED onto the video
# tokens, scaled by skeleton_embed_alpha.
#
...
# the runs' checkpoints and wandb projects never collide.
...
python src/maxdiffusion/train_wan.py \
    ...
    run_name=ac_wan_droid_skeleton_no_text \
    ...
    wandb_project='wan-ac-skeleton-no-text' \
...
# tpu create v6 --name train_ac_wan_skeleton_no_text -n 32 --setup-cmd "" --priority 0 --max-attempts 40 -- bash bash_scripts/train_ac_wan_skeleton_no_text.sh
