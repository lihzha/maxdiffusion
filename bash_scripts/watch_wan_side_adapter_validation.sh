...
VALIDATION_FORCE="${VALIDATION_FORCE:-0}"
...
echo "VALIDATION_FORCE=${VALIDATION_FORCE}"
...
_launch_validation() {
  ...
  local tpu_watch_args=(v6 -n "$VALIDATION_TPU_CHIPS")
  if [ "$VALIDATION_FORCE" = "1" ]; then
    tpu_watch_args+=(--force)
  fi
  tpu watch "${tpu_watch_args[@]}" \
    --setup-cmd "$setup_cmd" \
    "$WATCH_BRANCH" \
    RUN_NAME="$RUN_NAME" \
