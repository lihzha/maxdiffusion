...
if [ -f "$HOME/.config/irom-tpu/secrets.env" ]; then
  set +x
  # A WANDB_API_KEY forwarded from the submitting shell (job --env; launch_wan_train.sh's
  # W&B block) must survive the worker image's provisioned secrets: capture before sourcing,
  # restore after. Otherwise the provisioned key silently decides which W&B entity the run
  # lands in (2026-08-13; see CLAUDE.md "W&B entity facts"). No forwarded key => unchanged.
...
RUN_NAME="${RUN_NAME:-wan-overfit100-smoke}"
...
if [ ! -f "${MANIFEST_PATH}" ]; then
  echo "[overfit100] FATAL: manifest not found at MANIFEST_PATH=${MANIFEST_PATH}" >&2
  exit 1
fi
...
if ! printf '%s' "${MODEL_REVISION}" | grep -Eq '^[0-9a-f]{40}$'; then
  echo "[overfit100] FATAL: manifest revision '${MODEL_REVISION}' is not a 40-hex commit sha" >&2
  exit 1
fi
...
case "${MODEL_DIR}" in
  *"${MODEL_REVISION}"*) ;;
  *)
    echo "[overfit100] FATAL: resolved MODEL_DIR='${MODEL_DIR}' does not carry the pinned revision ${MODEL_REVISION}" >&2
    exit 1
    ;;
esac
