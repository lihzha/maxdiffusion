...
# equals the checked-in YAML's value -- except the run-identity and derived-storage keys, which are
# exempt BY NAME in the interface table the test file carries. A launcher that recipes differently
# from the config everyone reads is exactly the drift the SOP's parity audit exists for.
# =================================================================================================
...
echo "LEARNING_RATE=${LEARNING_RATE} SEED=${SEED} PER_DEVICE_BATCH_SIZE=${PER_DEVICE_BATCH_SIZE}"
echo "COMMIT=${COMMIT}"
