...
export RUN_TAG="${RUN_TAG:-skeleton-no-text}"
...
#
# THIS RUN: additive site. A separate zero-init conv embeds the skeleton latents
# and its output is ADDED onto conv_in`s output, scaled by skeleton_embed_alpha.
# SVD has no patch embedding, so conv_in`s output is the analogue of WAN`s
# patchified tokens: the earliest point in the network that is in feature space
# rather than latent space. Injected once, with the residual stream carrying it.
...
# tpu create v6 --name train_ac_svd_skeleton_no_text -n 32 --setup-cmd "" --priority 0 --max-attempts 40 -- bash bash_scripts/train_ctrl_world_skeleton_no_text.sh
